...
class DirectoryDataGenerator(tf.keras.utils.Sequence):
        ...
        for base_directory in base_directories:
            ...
            for cas in self.class_names:
                if len(cas) != len(class_names):
                    raise Exception("Directories do not have the same number of classes.")
            ...
            for i, c in enumerate(class_names):
                ...
                if isdir(class_dir):
                    # Only add valid image files (.png, .jpg, etc.)
                    valid_image_extensions = ('.jpg', '.jpeg', '.png', '.bmp')  # you can add more extensions as needed
                    for f in listdir(class_dir):
                        file_dir = join(class_dir, f)
                        if isfile(file_dir) and f.lower().endswith(valid_image_extensions):
    ...
    def __getitem__(self, index):
        ...
        # return [X,], y
        return X, y  # Return directly without wrapping X in a list
